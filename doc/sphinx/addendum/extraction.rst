...
.. cmd:: Set Extraction Conservative Types.
   ...
   types). Turn this option to Set to make sure that ``e:t``
   implies that ``e':t'`` where ``e'`` and ``t'`` are the extracted
   code of ``e`` and ``t`` respectively.

.. cmd:: Set Extraction KeepSingleton.
...
.. cmd:: Unset Extraction AutoInline.

   Default is ``Set``. The extraction mechanism inlines the bodies of
...
.. cmd:: Unset Extraction SafeImplicits.
...
.. cmd:: Extract Inlined Constant @qualid => @string.
   ...
   Note: this command is sugar for an ``Extract Constant`` followed
