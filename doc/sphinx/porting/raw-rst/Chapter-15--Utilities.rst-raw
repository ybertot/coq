...
+ `15.1 Building a toplevel extended with user tactics`_
+ `15.2 Building a Coq project with coq_makefile`_
+ `15.3 Modules dependencies`_
+ `15.4 Documenting Coq files with coqdoc`_
+ `15.5 Embedded Coq phrases inside L A T E X documents`_
+ `15.6 Coq and GNU Emacs`_
+ `15.7 Module specification`_
+ `15.8 Man pages`_
...
to debug your tactics with the Objective Caml debugger. You need to
...
To launch the Objective Caml debugger with the image you need to
execute it in an environment which correctly sets the COQLIB variable.
Moreover, you have to indicate the directories in whichocamldebug
...
See also: Section `14.3.3`_). Collecting the list of files and options
...
understand_CoqProject files and invoke Coq with the desired options.
...
: CoqMakefile: is a generic makefile for GNU Make that provides
...
:CoqMakefile.conf: contains make variables assignments that reflect
...
Extension points are documented in paragraph 15.2.
...
+ Objective Caml files that require pre processing for syntax
  extensions (like VERNAC EXTEND) must use the .ml4 extension
+ In order to generate a plugin one has to list all Objective Caml
...
reduces the chances of begin unable to load two distinct plugins
...
fileCoqMakefile. Such can contain two kinds of directives.

:Variable assignment: to the variables listed in the Parameters
section of the generated makefile. Here we describe only few of them.
    ...
    :COQC, COQDEP, COQDOC: can be set in order to use alternative binaries
      (e.g. wrappers)


:Rule extension: The following makefile rules can be extended. For
example
...
+ TIMED=1 — passing this variable will causemake to emit a line
  describing the user-space build-time and peak memory usage for each
  file built.Note: On Mac OS, this works best if you’ve installedgnu-
  time.Example: For example, the output of make TIMED=1 may look like
  this:
...
+ pretty-timed — this target stores the output ofmake TIMED=1 into
  ...
  TGTS="a.vo b.vo".Note: This target requires python to build the
  table.Note: This target will *append* to the timing log; if you want a
  fresh start, you must remove the filetime-of-build.log or run make
  cleanall.Example: For example, the output of make pretty-timed may
  look like this:
...
+ print-pretty-timed-diff — this target builds a table of timing
  ...
  repo.Note: This target requires python to build the table.Note: The
  make-pretty-timed-before andmake-pretty-timed-after targets will
  *append* to the timing log; if you want a fresh start, you must remove
  the filestime-of-build-before.log andtime-of-build-after.log or run
  make cleanall *before* building either the “before” or “after”
  targets.Note: The table will be sorted first by absolute time
  ...
  files which take effectively no time to compile.Example: For example,
...
+ TIMING=1 — passing this variable will causemake to use coqc -time to
  write to a.v.timing file for each .v file compiled, which contains
  line-by-line timing information.Example: For example, running make all
  TIMING=1 may result in a file like this:
...
+ print-pretty-single-time-diff BEFORE=path/to/file.v.before-timing
  AFTER=path/to/file.v.after-timing — this target will make a sorted
  table of the per-line timing differences between the timing logs in
  the BEFORE and AFTER files, display it, and save it to the file
  specified by theTIME_OF_PRETTY_BUILD_FILE variable, which defaults
  totime-of-build-pretty.log.To generate the .v.before-timing or.v
  .after-timing files, you should passTIMING=before or TIMING=after
  rather thanTIMING=1.Note: The sorting used here is the same as in
  theprint-pretty-timed-diff target.Note: This target requires python to
  build the table.Example: For example, runningprint-pretty-single-time-
  diff might give a table like this:
...
+ all.timing.diff, path/to/file.v.timing.diff — The
  path/to/file.v.timing.diff target will make a.v.timing.diff file for
  the corresponding .v file, with a table as would be generated by
  theprint-pretty-single-time-diff target; it depends on having already
  made the corresponding .v.before-timing and.v.after-timing files,
  which can be made by passingTIMING=before and TIMING=after.
  Theall.timing.diff target will make such timing difference files for
  all of the .v files that the Makefile knows about. It will fail if
  some .v.before-timing or.v.after-timing files don’t exist.Note: This
...
One way to get access to all targets of the generatedCoqMakefile is to
...
coqdep computes inter-module dependencies for Coq andObjective Caml
...
Objective Caml modules dependencies.
...
coqdoc is a documentation tool for the proof assistantCoq, similar to
javadoc or ocamldoc. The task of coqdoc is


#. to produce a nice L A T E X and/or HTML document from the Coq
...
It gives the L A T E X and HTML texts to be produced for the given Coq
token. One of the L A T E X or HTML text may be omitted, causing the
...
`->` → `<-` ← `*` × `<=` ≤ `>=` ≥ `=>` ⇒ `<>` ≠ `<->` ↔ `|-` ⊢ `\/` ∨
`/\` ∧ `~` ¬
Any of these can be overwritten or suppressed using theprinting
commands.
...
Escaping to L A T E X and HTML.
+++++++++++++++++++++++++++++++

Pure L A T E X or HTML material can be inserted using the following
escape sequences:


+ `$...LaTeX stuff...$` inserts some L A T E X material in math mode.
  Simply discarded in HTML output.
+ `%...LaTeX stuff...%` inserts some L A T E X material. Simply
...
Then invoke coqdoc or coqdoc --glob-from *file* to tellcoqdoc to look
...
suffixes `.v` and `.g` and L A T E X files by the suffix `.tex`.
...
:L A T E X output: A single L A T E X file is created, on standard
  output. It can be redirected to a file with option -o. The order of
  files on the command line is kept in the final document. L A T E X
...
:T E Xmacs output: To translate the input files to T E Xmacs format,
  to be used by the T E Xmacs Coq interface.
...
:--latex: Select a L A T E X output.
:--dvi: Select a DVI output.
:--ps: Select a PostScript output.
:--texmacs: Select a T E Xmacs output.
...
:-p string, --preamble string: Insert some material in the L A T E X
...
:-toc, --table-of-contents: Insert a table of contents. For a L A T E
  X output, it inserts a `\tableofcontents` at the beginning of the
...
:-R dir coqdir: Map physical directory dir to Coq logical
  directorycoqdir (similarly to Coq option -R).Note: option -R only has
...
:-l, --light: Light mode. Suppress proofs (as with -g) and the
following commands:
...
:--interpolate: Use the globalization information to typeset
  identifiers appearing inCoq escapings inside comments.
...
:-utf8, --utf8: Set --inputenc utf8x for L A T E X output and--charset
  utf-8 for HTML output. Also use Unicode replacements for a couple of
  standard plain ASCII notations such as → for -> and ∀ forforall. L A T
  E X UTF-8 support can be found
  at`http://www.ctan.org/pkg/unicode`_.For the interpretation of Unicode
  characters by L A T E X, extra packages which coqdoc does not provide
  by default might be required, such as textgreek for some Greek letters
  orstmaryrd for some mathematical symbols. If a Unicode character is
...
:--inputenc string: Give a L A T E X input encoding, as an option to L
  A T E X packageinputenc.
:--charset string: Specify the HTML character set, to be inserted in
  the HTML header.



15.4.3 The coqdoc L A T E X style file
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

In case you choose to produce a document without the default L A T E X
...
:coqdocmodule: One-argument macro for typesetting the title of a `.v`
file. Default is
...
15.5 Embedded Coq phrases inside L A T E X documents
----------------------------------------------------

When writing a documentation about a proof development, one may want
to insert Coq phrases inside a L A T E X document, possibly together
with the corresponding answers of the system. We provide a mechanical
way to process such Coq phrases embedded in L A T E X files: thecoq-
tex filter. This filter extracts Coq phrases embedded in LaTeX files,
...
Remark. This Reference Manual and the Tutorial have been completely
produced with coq-tex.
...
Coq. It is freely available at `https://proofgeneral.github.io/`.
...
There are man pages for the commands coqdep, gallina andcoq-tex. Man
...
.. _15.8  Man pages: :///home/steck/tools.html#ManPages
.. _Table of contents: :///home/steck/toc.html
.. _Community: :///community
.. _14.3.3: :///home/steck/commands.html#coqoptions
.. _15.3  Modules dependencies: :///home/steck/tools.html#Dependencies
.. _http://coq.inria.fr/library/: http://coq.inria.fr/library/
.. _X documents: :///home/steck/tools.html#Latex
.. _webmaster: mailto:coq-www_@_inria.fr
.. _Commands: :///home/steck/command-index.html
.. _15.7  Module specification: :///home/steck/tools.html#gallina
.. _ files with coqdoc: :///home/steck/tools.html#coqdoc
...
.. _GNU Emacs: :///home/steck/tools.html#Emacs
.. _coq_makefile: :///home/steck/tools.html#Makefile
.. _Documentation: :///documentation
.. _15.1  Building a toplevel extended with user tactics: :///home/steck/tools.html#Coqmktop
.. _http://www.ctan.org/pkg/unicode: http://www.ctan.org/pkg/unicode
