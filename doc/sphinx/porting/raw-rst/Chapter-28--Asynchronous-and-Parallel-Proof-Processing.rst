...
+ `28.1 Proof annotations`_
+ `28.2 Proof blocks and error resilience`_
+ `28.3 Interactive mode`_
+ `28.4 Batch mode`_
+ `28.5 Limiting the number of parallel workers`_
...
Windows. It can be enabled, as described below at 28.3, though doing
...
theProof using command (Section `7.1.5`_) that declares which section
...
The command Set Suggest Proof Using makes Coq suggest, when aQed
...
Coq 8.6 introduces a mechanism for error resiliency: in interactive
...
:curly: blocks are delimited by { and }, see `7`_
...
commands can be switched off passing-async-proofs-command-error-
resilience off to CoqIDE.
...
options:-async-proofs-tactic-error-resilience off,-async-proofs-
tactic-error-resilience all,-async-proofs-tactic-error-resilience
blocktype 1 ,..., blocktype n . Valid proof block types are: “curly”,
“par”, “indent”, “bullet”.
...
to a worker process. The threshold can be configure with -async-
proofs-delegation-threshold. Default is 0.03 seconds.


28.4 Batch mode
---------------

When Coq is used as a batch compiler by running coqc orcoqtop
...
compiled file (like the .vo one) that can be loaded byRequire from the
generation and checking of the proof objects. The -quick flag can be
passed to coqc orcoqtop to produce, quickly, .vio files.
Alternatively, when using a Makefile produced by coq_makefile,
thequick target can be used to compile all files using the-quick flag.
...
obtain a good scheduling for 2 workers to produce a.vo, b.vo, and
c.vo. When using a Makefile produced by coq_makefile, thevio2vo target
...
b.vio, andc.vio. Auxiliary files are used to predict how long a proof
...
likeCOQWORKMGR_SOCKET=localhost:45634. The user must set this variable
in all the shells from which Coq processes will be started. If one
uses just one terminal running the bash shell, then export ‘coqworkmgr
-j 4‘ will do the job.
...
.. _28.3  Interactive mode: :///home/steck/async-proofs.html#sec874
.. _28.5  Limiting the number of parallel workers: :///home/steck/async-proofs.html#sec877
.. _28.4  Batch mode: :///home/steck/async-proofs.html#sec876
.. _28.1  Proof annotations: :///home/steck/async-proofs.html#sec870
.. _7: :///home/steck/proof-handling.html#Proof-handling
.. _xhtml valid: http://validator.w3.org/
.. _Tactics: :///home/steck/tactic-index.html
.. _28.2  Proof blocks and error resilience: :///home/steck/async-proofs.html#sec872
